...
export class AppComponent {
  ...
  conseguirTipo(): string[] {
    const tipo = this.fotos.map(value => value.tipo);
    return tipo.filter((value: string, idx: number) => tipo.indexOf(value) == idx);
  }
  
  obtenerFotosPorTipo(): Foto[] {
    return this.fotos.filter(value => (value.tipo.toLowerCase()).indexOf(this.tipo.toLowerCase()) != -1);
  }

  guardarFoto(foto: Foto) {
    console.log(foto);
    // Os falta hacer la substitución de objeto en el array this.fotos
  }
